...
5.  **Phase 2: Generate Sub-Tasks:** Once the user confirms, break down each parent task into smaller, actionable sub-tasks necessary to complete the parent task. Ensure sub-tasks logically follow from the parent task and cover the implementation details implied by the requirements. All Tasks should hae a maximum number of 6 sub-tasks. If more sub-tasks are needed to build the necessary complexity and get the work complete, then either 1) break subasks down into another level of subtasks as needed to keep the coding difficulty to a low enough level of complexity so that a Composer2.5 level-coding agent can handle it relatively easiely, 2) move additional work into another main task with it's own sub-tasks, or 3) some combination of both.
